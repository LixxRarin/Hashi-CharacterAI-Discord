...
class discord_AI_bot:
    ...
    async def read_channel_messages(self, message, client):
        ...
        try:
            ...
            channels = [
                channel_id
                for server_data in webhook.session_data.values()
                for channel_id in server_data
            ]

            # Check if message is in a monitored channel and not from the bot.
            if (str(message.channel.id) in channels) and \
                ...
                if message.reference is not None:
                    try:
                        ...
                        utils.capture_message(
                            "messages_cache.json", message, ref_message)
    ...
    async def AI_send_message(self, client, message):
        """
            ...
            if utils.test_internet():
                try:
                    async with response_channel.typing():
                        ...
                        response = await cai.cai_response(cached_data, message)
                    ...
                    self.last_message_time = time.time()
                    try:
                        # Lê o cache atual; se não existir ou não for um dicionário, inicializa como dict.
                        current_cache = utils.read_json(
                            utils.config_yaml["Discord"]["messages_cache"])
                        if current_cache is None or not isinstance(current_cache, dict):
                            current_cache = {}

                        # Itera sobre os servidores presentes em cached_data
                        for server_id, channels in cached_data.items():
                            if server_id in current_cache:
                                # Itera sobre os canais para esse servidor
                                for channel_id, keys in channels.items():
                                    if channel_id in current_cache[server_id]:
                                        # Remove cada mensagem (ou reply) processada
                                        for key in keys:
                                            if key in current_cache[server_id][channel_id]:
                                                del current_cache[server_id][channel_id][key]
                                        # Opcional: se o canal ficar vazio, pode removê-lo
                                        if not current_cache[server_id][channel_id]:
                                            del current_cache[server_id][channel_id]
                                # Opcional: se o servidor ficar sem canais, pode removê-lo
                                if not current_cache[server_id]:
                                    del current_cache[server_id]

                        utils.write_json(
                            utils.config_yaml["Discord"]["messages_cache"], current_cache)
                        utils.log.debug(
            ...
            if not self.awaiting_response:
                try:
                    ...
                    if (time.time() - self.last_message_time >= 7 or cache_count >= 5) and cache_count >= 1:
                        ...
                        await self.AI_send_message(client, message)
                except Exception as e:
